...
let day = new Date("2017-02-01T12:00:00");
const daysInMonth = 
    moment(day)
...
const shiftsInMonth = daysInMonth * 2;
let userArray = R.range(0, shiftsInMonth);

function Shift (start, end, user) {
    return { start, end, title: "Unavailable", color: "red" };
}
function Schedule () {
  this.scheduleArray = [];
...
console.log(day);
  ...
  period.add(new Shift(start, end, user));
...
delete period.add;
let output = {}
output.template = period.scheduleArray;
fs.writeFileSync('shift-template.json', JSON.stringify(output));
